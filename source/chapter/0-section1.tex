...
\begin{enumerate}[label=\alph*\upshape)]
	\item $x \in C_{c}^{2}(\R^{d}) : \Delta x = \sum_{j = 1}^{d} \frac{\partial^{2}}{\partial x_{j}^{2} x(u)} \in L^{2}(\R^{d})$
		...
		%$\hat \Delta : \underbrace{C_{c}^{2}(\R^{d}, \| \cdot \|_{\Delta})^{\hat}}_{\underset{schwache Ableitung}{\hat \Delta \colon H^{2, 2}(\Omega)} \rightarrow L^{2}(\underset{abgeschlossen}{\R^{d}})} \rightarrow L^{2}(\R^{d})$
